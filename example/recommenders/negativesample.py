...
class NegativeSamplingDataIter(mx.io.DataIter):
    ...
    def _push_queue(self, data_list, labels):
        ...
        for idx in range(1+self.sample_ratio):
            nddata = [mx.nd.array(slicer(x, idx)) for x in data_list]
            ndlabels = mx.nd.array(slicer(labels, idx))
    ...
    def _shuffle_batch(self, data):
        ...
        num_batch_size = data[0].shape[0]
        si = np.arange(num_batch_size)
        np.random.shuffle(si)
        matches = (si == np.arange(num_batch_size)) # everywhere it didn't shuffle
    ...
    def _refill_queue(self):
        ...
        num_batch_size = original[0].shape[0]
        ...
        outlabels = [np.ones(num_batch_size) * self.positive_label] # list of numpy arrays
        ...
        for _ in range(self.sample_ratio):
            ...
            for idx, d in enumerate(shuffled):
                outdata[idx].append(d)
            outlabels.append(np.ones(num_batch_size) * self.negative_label)
...
if __name__ == "__main__":
    ...
    for ratio in range(0, 5):
        ...
        bat = nsdi.next()
        for i, element in bat.data:
            assert element.shape[0] == batch_size
            assert element.shape[1] == oribatch.data[i].shape[1]
