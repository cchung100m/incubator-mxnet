...
# under the License.
"""
The RBM is implemented as a custom operator, and a gluon block is also provided.
binary_rbm.py contains the implementation of the RBM.
"""
import ast
import mxnet as mx
...
class BinaryRBM(mx.operator.CustomOp):
    """
    Create behavior methods of Binary RBM
    """
    def __init__(self, k):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        """
        Generate forward propagation of RBM
        """
        ...
        if is_train:
            _, hidden_layer_prob_1 = self.sample_hidden_layer(visible_layer_data,
                                                              hidden_layer_bias,
                                                              interaction_weight)
            hidden_layer_sample = aux[1]  # The initial state of the Gibbs sampling for persistent CD
        else:
            hidden_layer_sample, hidden_layer_prob_1 = self.sample_hidden_layer(visible_layer_data,
                                                                                hidden_layer_bias,
                                                                                interaction_weight)

        # k-step Gibbs sampling
        for _ in range(self.k):
            visible_layer_sample, visible_layer_prob_1 = self.sample_visible_layer(hidden_layer_sample,
                                                                                   visible_layer_bias,
                                                                                   interaction_weight)
            hidden_layer_sample, _ = self.sample_hidden_layer(visible_layer_sample,
                                                              hidden_layer_bias,
                                                              interaction_weight)
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        """
        Generate backward propagation of RBM
        """
        visible_layer_data = in_data[0]    # (num_batch, num_visible)
        ...
        grad_interaction_weight = (mx.nd.linalg.gemm2(visible_layer_sample.expand_dims(2),
                                                      hidden_layer_sample.expand_dims(1)) -
                                   mx.nd.linalg.gemm2(visible_layer_data.expand_dims(2),
                                                      hidden_layer_prob_1.expand_dims(1))
                                   ).mean(axis=0)
    ...
    def sample_layer(self, other_layer_sample, layer_bias, interaction_weight, interaction_transpose):
        prob_1 = mx.nd.linalg.gemm(other_layer_sample,
                                   interaction_weight,
...
class BinaryRBMProp(mx.operator.CustomOpProp):
    """
    Generate helper functions of BinaryRBM
    """
    ...
    def list_outputs(self):
        # 0: The probabilities that each visible unit is 1 after `k` steps of
        # Gibbs sampling starting from the given `data`.
...
class BinaryRBMBlock(mx.gluon.HybridBlock):
    """
    Generate binary RBM class for gluon API
    """
    def __init__(self, num_hidden, k, for_training, **kwargs):
        ...
        with self.name_scope():
            ...
            if for_training:
                self.aux_visible_layer_sample = self.params.get('aux_visible_layer_sample', shape=(0, 0),
                                                                allow_deferred_init=True)
                self.aux_hidden_layer_sample = self.params.get('aux_hidden_layer_sample', shape=(0, 0),
                                                               allow_deferred_init=True)

    def hybrid_forward(self, F, data, visible_layer_bias, hidden_layer_bias, interaction_weight,
                       aux_visible_layer_sample=None, aux_hidden_layer_sample=None):
        """
        Generate hybrid_forward(aux_visible_layer_sample, aux_hidden_layer_sample) based on training constant
        """
...
def estimate_log_likelihood(visible_layer_bias, hidden_layer_bias, interaction_weight, ais_batch_size, ais_num_batch,
                            ais_intermediate_steps, ais_burn_in_steps, data, ctx):
    """
    Use the annealed importance sampling to monitor the progress of the learning
    """
    ...
    def log_intermediate_unnormalized_prob(visible_layer_sample, beta):
        p = mx.nd.dot(visible_layer_sample, (1 - beta) * base_rate_visible_layer_bias + beta * visible_layer_bias)
        if beta != 0:
            p += mx.nd.linalg.gemm(visible_layer_sample,
                                   interaction_weight,
