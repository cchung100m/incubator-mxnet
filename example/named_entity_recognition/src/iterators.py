...
"""
Generate iterator for feature/label array
"""
...
class BucketNerIter(DataIter):
    ...
    def __init__(self, sentences, characters, label, max_token_chars, batch_size, buckets=None,
                 data_pad=-1, label_pad=-1, data_names=None,
        ...
        if nslice > 0:
            print("Warning, {0} sentences sliced to largest bucket size.".format(nslice))
        ...
        self.provide_data = [DataDesc(name=self.data_names[0], shape=(self.batch_size, self.default_bucket_key),
                                      layout=self.layout),
                             DataDesc(name=self.data_names[1], shape=(self.batch_size, self.default_bucket_key,
                                                                      self.max_token_chars), layout=self.layout)]
        self.provide_label = [DataDesc(name=self.label_name, shape=(self.batch_size, self.default_bucket_key),
                                       layout=self.layout)]
        ...
        for i, buck in enumerate(self.sentences):
            # extend the list eg output with batch size 5 and 20 training examples in bucket.
            # [(0,0), (0,5), (0,10), (0,15), (1,0), (1,5), (1,10), (1,15)]
