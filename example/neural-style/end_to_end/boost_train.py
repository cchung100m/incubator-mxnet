...
# under the License.
"""
Train Deep Neural Network for art transformation
"""
import os
import random
import logging
...
for gen in gens:
    gen.init_optimizer(optimizer='sgd',
...
# tv-loss
def get_tv_grad_executor(img, context, num_tv_weight):
    """create TV gradient executor with input binded on img
    """
    if num_tv_weight <= 0.0:
        return None
    ...
    kernel = mx.nd.array(np.array([[0, -1, 0],
                         ...
                         .reshape((1, 1, 3, 3)),
                         context) / 8.0
    out = out * num_tv_weight
    return out.bind(context, args={"img": img, "kernel": kernel})
...
for i in range(start_epoch, end_epoch):
    ...
    for idx in range(num_image):
        ...
        try:
            ...
            logging.warning("Fail to load an input image. Skip.")
        ...
        # gen_forward
        for k, element in enumerate(gens):
            element.forward(mx.io.DataBatch([data_array[-1]], [0]), is_train=True)
            data_array.append(element.get_outputs()[0].copyto(mx.cpu()))
            # loss forward
        ...
        for k in range(len(gens) - 1, -1, -1):
            tv_grad_executor = get_tv_grad_executor(gens[k].get_outputs()[0], ctx, tv_weight)
        ...
        if idx % 20 == 0:
            ...
            for k, element in enumerate(gens):
                logging.info("Data Norm :%.5f", (mx.nd.norm(element.get_input_grads()[0]).asscalar() / np.prod(dshape)))
        if idx % 1000 == 0:
            for k, element in enumerate(gens):
                element.save_params("./model/%d/%s_%04d-%07d.params" % (k, model_prefix, i, idx))
