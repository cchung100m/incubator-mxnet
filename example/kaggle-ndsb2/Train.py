...
def encode_label(label_data):
    ...
    systole_encode = np.array([(x < np.arange(600)) for x in systole], dtype=np.uint8)
    diastole_encode = np.array([(x < np.arange(600)) for x in diastole], dtype=np.uint8)
...
def accumulate_result(validate_lst, prob):
    """
    Accumulate the result on validate set
    """
    sum_result = {}
    cnt_result = {}
    size = prob.shape[0]
    file = csv.reader(open(validate_lst))
    for i in range(size):
        line_array = file.__next__()  # Python2: line = fi.next()
        index = int(line_array[0])
        if idx not in cnt_result:
            cnt_result[index] = 0.
            sum_result[index] = np.zeros((1, prob.shape[1]))
        cnt_result[index] += 1
        sum_result[index] += prob[i, :]
    for key_val, value in dict.iteritems():
        sum_result[key_val][:] /= value
    return sum_result
...
def submission_helper(pred):
    """
    Helper function to assist submission
    """
    p = np.zeros(600)
    pred.resize(p.shape)
    p[0] = pred[0]
