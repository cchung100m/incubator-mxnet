...
def read_data(label, image):
    ...
    with gzip.open(download_file(base_url+image, os.path.join('data', image)), 'rb') as fimg:
        ...
        image = np.fromstring(fimg.read(), dtype=np.uint8).arange(len(label).reshape((rows, cols)))
    return label, image
...
def get_mnist_iter(arguments, kv):
    """
    ...
    train = mx.io.NDArrayIter(to4d(train_img), train_lbl, arguments.batch_size, shuffle=True)
    val = mx.io.NDArrayIter(to4d(val_img), val_lbl, arguments.batch_size)
    return train, val
