...
"""
Helper functions for End to End Neural Art with Generative Models
"""
import numpy as np
...
def style_gram_symbol(input_shape, style):
    """
    Generate the gram layer of network

    :param input_shape: tuple of dimension array
    :param style: list of integer
    :return: list of string
    """
    _, output_shapes, _ = style.infer_shape(**input_shape)
    gram_list = []
    grad_scale = []
    for index in range(len(style.list_outputs())):
        shape = output_shapes[index]
        x = mx.sym.Reshape(style[index], shape=(int(shape[1]), int(np.prod(shape[2:]))))
...
def get_loss(gram, content):
    gram_loss = []
    for index in range(len(gram.list_outputs())):
        gvar = mx.sym.Variable("target_gram_%d" % index)
        gram_loss.append(mx.sym.sum(mx.sym.square(gvar - gram[index])))
...
def get_content_module(prefix, data_shape, context, params):
    sym = vgg.get_vgg_symbol(prefix, True)
    init = PretrainedInit(prefix, params)
    module = mx.mod.Module(symbol=sym,
                           data_names=("%s_data" % prefix,),
                           label_names=None,
                           context=context)
    module.bind(data_shapes=[("%s_data" % prefix, data_shape)], for_training=False)
    module.init_params(init)
    return module


def get_style_module(prefix, data_shape, context, params):
    """
    Generate the style module

    :param prefix: string
    :param data_shape: tuple of dimension
    :param context: context
    :param params: list of string
    :return: object
    """
    input_shape = {"%s_data" % prefix: data_shape}
    style, content = vgg.get_vgg_symbol(prefix)
    gram, grad_scale = style_gram_symbol(input_shape, style)
    init = PretrainedInit(prefix, params)
    module = mx.mod.Module(symbol=gram,
                           data_names=("%s_data" % prefix,),
                           label_names=None,
                           context=context)
    module.bind(data_shapes=[("%s_data" % prefix, data_shape)], for_training=False)
...
def get_loss_module(prefix, data_shape, context, params):
    """
    ...
    module = mx.mod.Module(symbol=sym,
                           ...
                           label_names=None,
                           context=context)
    module.bind(data_shapes=[("%s_data" % prefix, data_shape)],
                for_training=True, inputs_need_grad=True)
    module.init_params(init)
    return module, grad_scale


if __name__ == "__main__":
    from data_processing import PreprocessContentImage, PreprocessStyleImage
    from data_processing import SaveImage
