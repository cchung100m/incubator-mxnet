...
def get_data(lst, preproc):
    """
    Read data from file directory and reshape the data set
    """
    data = []
    result = []
    for path in lst:
...
def local_split(train_idx):
    random.seed(0)
    train_idx = set(train_idx)
    all_index = sorted(train_idx)
    num_test = int(len(all_index) / 3)
    random.shuffle(all_index)
    new_train_set = set(all_index[num_test:])
    new_test_set = set(all_index[:num_test])
    return new_train_set, new_test_set


def split_csv(src_csv, split_to_train_list, train_csv, test_csv):
    """
    Split the dataset to train set
    """
    ftrain = open(train_csv, "w")
    ftest = open(test_csv, "w")
    cnt = 0
    for l in open(src_csv):
        if split_to_train_list[cnt]:
            ftrain.write(l)
        else:
            ftest.write(l)
        cnt = cnt + 1
