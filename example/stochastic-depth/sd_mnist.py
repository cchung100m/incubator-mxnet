...
################################################################################
"""
shows sample implementation of the algorithm just for the sanity check.
"""
import os
import logging
import mxnet as mx
...
def get_conv(name, data, num_filter, kernel, stride, pad, with_relu, bn_momentum):
    conv_layer = mx.symbol.Convolution(name=name, data=data, num_filter=num_filter, kernel=kernel, stride=stride,
                                       pad=pad, no_bias=True)
    bn = mx.symbol.BatchNorm(name=name + '_bn', data=conv_layer, fix_gamma=False, momentum=bn_momentum,
...
conv = get_conv(name='conv0', data=input_data, num_filter=16, kernel=(3, 3), stride=(1, 1), pad=(1, 1), with_relu=True,
                bn_momentum=0.9)
...
for i, element in enumerate(death_rates):
    conv = get_conv(name='conv0_%d' % i, data=mx.sym.Variable('data_%d' % i), num_filter=16, kernel=(3, 3),
                    stride=(1, 1), pad=(1, 1), with_relu=True, bn_momentum=0.9)
    conv = get_conv(name='conv1_%d' % i, data=conv, num_filter=16, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                    with_relu=False, bn_momentum=0.9)
    mod = sd_module.StochasticDepthModule(conv, data_names=['data_%d' % i], context=contexts, death_rate=element)
...
train = mx.io.MNISTIter(image=os.path.join(basedir, "data", "train-images-idx3-ubyte"),
...
val = mx.io.MNISTIter(image=os.path.join(basedir, "data", "t10k-images-idx3-ubyte"),
