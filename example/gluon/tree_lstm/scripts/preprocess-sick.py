...
def dependency_parse(filepath, cp='', tokenize=True):
    """
    generate dependency parses using Stanford Neural Network Dependency Parser for
...
def build_vocab(filepaths, dst_path, lowercase=True):
    """
    helper function to turn the vocab to lowercase for handling SICK dataset (semantic relatedness task)

    :param filepaths: list of string
        multiple directory to files
    :param dst_path: string
        destination file path to create new file
...
def split(filepath, dst_dir):
    """
    helper function to split different file name from one line for handling SICK dataset (semantic relatedness task)
