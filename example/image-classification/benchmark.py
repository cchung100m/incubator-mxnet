...
"""
Run benchmark to evaluate the CNN on image classification
"""
...
def setup_logging(log_location):
    ...
    if os.path.exists(log_location):
        shutil.move(log_location, log_location + "_" + str(int(os.path.getctime(log_location))))
    os.makedirs(log_location)

    log_file = '{}/benchmark.log'.format(log_location)
...
class RunCmd(threading.Thread):
    """
    Runs the command given in the cmd_args for specified timeout period and terminates after
    """
...
def parse_args():
    ...
    :return: revised arguments
    """
    class NetworkArgumentAction(argparse.Action):
        """
        Create helper functions to validate network argument and object constructor
        """
        def validate(self, attrs):
            ...
            """
            args = attrs.split(':')
            if len(args) != 4 or not isinstance(args[0], str) or not isinstance(args[1], str):
    ...
    parser.add_argument('--networks', dest='networks', nargs='+', type=str,
                        help='one or more networks in the format mode:network_name:batch_size:image_size '
                             'The network_name is a valid model defined as network_name.py in the image-classification'
                             'symbol folder for native imagenet '
                             'Or a gluon vision model defined in mxnet/python/mxnet/gluon/model_zoo/model_store.py.',
                        action=NetworkArgumentAction)
    parser.add_argument('--worker_file', type=str,
                        help='file that contains a list of worker hostnames or list of worker ip addresses '
                             'that can be sshed without a password.',
                        required=True)
...
def images_processed(log_location, mode):
    """
    Choose the middle iteration to get the images processed per sec

    :param log_location: string
    :param mode: string
    :return: int
    """
    f = open(log_location)
...
def run_benchmark(kv_store, data_shape, batch_size, num_gpus, num_nodes, network, args_workers_file, mode):
    """
    Run benchmark to process images
    """
    if mode == 'native':
...
def plot_graph(args):
    """
    Helper function to illustrate graph
    """
    ...
    for net in args.networks:
        ...
        speedup_chart.add(net.name, y_values,
                          formatter=lambda y_val, img=copy.deepcopy(image_single_gpu),
                                           batch_size=copy.deepcopy(net.batch_size):
                          'speedup:%.2f, img/sec:%.2f, batch/gpu:%d' % (0 if y_val is None else y_val, 0
                                                                        if y_val is None else y_val * img, batch_size))
    speedup_chart.render_to_file(log_loc + '/speedup.svg')


def write_csv(log_location, args):
    for net in args.networks:
        with open(log_location + '/' + net.name + '.csv', 'wb') as f:
            w = csv.writer(f)
...
def main():
    ...
    for net in args.networks:
        ...
        LOGGER.info('Network: %s (num_gpus, images_processed): %s',
                    net.name, ','.join(map(str, net.gpu_speedup.items())))
