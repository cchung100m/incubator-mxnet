...
"""
Generate VGG19 model for neural style translation
"""
...
def get_vgg_symbol(prefix, content_only=False):
    """
    Construct the VGG 19 model for neural style translation

    :param prefix: string
    :param content_only: boolean
    """
    # declare symbol
    data = mx.sym.Variable("%s_data" % prefix)
    conv1_1 = mx.symbol.Convolution(name='%s_conv1_1' % prefix, data=data, num_filter=64, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu1_1 = mx.symbol.Activation(data=conv1_1, act_type='relu')
    conv1_2 = mx.symbol.Convolution(name='%s_conv1_2' % prefix, data=relu1_1, num_filter=64, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    ...
    conv2_1 = mx.symbol.Convolution(name='%s_conv2_1' % prefix, data=pool1, num_filter=128, pad=(1, 1),
                                    kernel=(3, ), stride=(1, 1), workspace=1024)
    relu2_1 = mx.symbol.Activation(data=conv2_1, act_type='relu')
    conv2_2 = mx.symbol.Convolution(name='%s_conv2_2' % prefix, data=relu2_1, num_filter=128, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    ...
    conv3_1 = mx.symbol.Convolution(name='%s_conv3_1' % prefix, data=pool2, num_filter=256, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu3_1 = mx.symbol.Activation(data=conv3_1, act_type='relu')
    conv3_2 = mx.symbol.Convolution(name='%s_conv3_2' % prefix, data=relu3_1, num_filter=256, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu3_2 = mx.symbol.Activation(data=conv3_2, act_type='relu')
    conv3_3 = mx.symbol.Convolution(name='%s_conv3_3' % prefix, data=relu3_2, num_filter=256, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu3_3 = mx.symbol.Activation(data=conv3_3, act_type='relu')
    conv3_4 = mx.symbol.Convolution(name='%s_conv3_4' % prefix, data=relu3_3, num_filter=256, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    ...
    conv4_1 = mx.symbol.Convolution(name='%s_conv4_1' % prefix, data=pool3, num_filter=512, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu4_1 = mx.symbol.Activation(data=conv4_1, act_type='relu')
    conv4_2 = mx.symbol.Convolution(name='%s_conv4_2' % prefix, data=relu4_1, num_filter=512, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu4_2 = mx.symbol.Activation(data=conv4_2, act_type='relu')
    conv4_3 = mx.symbol.Convolution(name='%s_conv4_3' % prefix, data=relu4_2, num_filter=512, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    relu4_3 = mx.symbol.Activation(data=conv4_3, act_type='relu')
    conv4_4 = mx.symbol.Convolution(name='%s_conv4_4' % prefix, data=relu4_3, num_filter=512, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
    ...
    conv5_1 = mx.symbol.Convolution(name='%s_conv5_1' % prefix, data=pool4, num_filter=512, pad=(1, 1),
                                    kernel=(3, 3), stride=(1, 1), workspace=1024)
...
def get_executor_with_style(style, content, input_size, ctx):
    """
    Get executor style of VGG 19 model
    """
...
def get_executor_content(content, input_size, ctx):
    """
    Get executor content of VGG 19 model
    :return:
    """
